...
#4th option in the main menu
        ...
        if option == 1:
            while True:
                ...
                if len(my_list) > 1:
                    ...
                    while id_option not in my_list:
                        id_option = get_integer("Choose one of the above students based on their studentid: ")
                    print(f"You chose to delete the student with id: {id_option}")
    ...
    def print_students_names(self):
        ...
        for i in range(len(self.students_list)):
            print(f"{self.students_list[i]["surname"]} {self.students_list[i]["name"]} of {self.students_list[i]["fathersname"][0]}")
...
#8th option in the main menu
    ...
    def print_teachers_names(self):
        ...
        for i in range(len(self.teachers_list)):
            print(f"{self.teachers_list[i]["surname"]} {self.teachers_list[i]["name"]} of {self.teachers_list[i]["fathersname"][0]}")
    ...
    def search_teacher_by_id(self):
        id_option = get_integer("Give the id of the teacher you want to find: ")
        while True:
            for teacher in self.teachers_list:
                if teacher["teacherid"] == id_option:
                    return id_option
            id_option = get_integer("There is not a teacher with this id on the system, please insert again an id: ")
